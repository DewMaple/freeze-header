/*
 ...
 Copyright (c) 2015 lee zhang  Version 0.6.2
 Copyright (c) 2011 Brent Muir Version 0.6.1

 Licensed under the MIT license ( http://www.opensource.org/licenses/mit-license )

 Version 0.6.2
 Change log:
 - fix bug of incompatibility of jQuery v1.11.1
 - add offset support
 - fix bug of calculate clone table width
...
(function ($) {
    ...
    var methods = {
        ...
        init: function (options) {
            ...
            return this.each(function () {
                ...
                if (!data) {
                    ...
                    var $firstTh = $this.find('th:eq(0)');

                    if (settings.left) {
                        leftHeader = $this.clone(false)
                            .find("th:nth-child(n+2), td:nth-child(n+2)").remove().end()
                            .css({'width': $firstTh.width(), 'background-color': $firstTh.css('background-color')})
                            .appendTo(document.body).wrap("<div>").parent()
                            .attr('data-offset', settings.offsetLeft)
                    ...
                    if (settings.top) {
                        topHeader = $this.clone(false)
                            ...
                            .wrap("<div>").parent()
                            .attr('data-offset', settings.offsetTop)
                            .css({position: 'absolute', top: top, left: left});
                    }
                    ...
                    if (settings.left && settings.top) {
                        cornerHeader = topHeader.clone(false)// skip a few steps by cloning topHeader
                            ...
                            .appendTo(document.body);
                        cornerHeader.find('table').css('width', $firstTh.width());
                    }
        ...
        resize: function (event) {
            ...
            if (leftHeader) {
                leftHeader.find('table tr').each(function (i) {
                    var tdDivs = $(table.find('tr').eq(i)).children().children();
                    var maxHeight = 0;
                    var height = 0;
                    var width = 0;
        ...
        scroll: function (event) {
            ...
            if (topHeader) {
                ...
                // - topHeader.height()
                // - table.find('tr:last').offsetHeight;
                var offsetTop = topHeader.data('offset');
            }
            if (leftHeader) {
                var tableRight = tableLeft + table.width();
                // - leftHeader.width()
                // - table.find('tr:first td:last').offsetWidth;
                var offsetLeft = leftHeader.data('offset');
            }
                ...
                if (topHeader) {
                    topHeader.css(
                        ...
                        {visibility: 'hidden'} :
                        {
                            visibility: 'visible',
                            position: 'fixed',
                            top: Math.min(Math.max(0, tableTop - scrollTop), tableBottom - scrollTop) + offsetTop,
                            // top: 0,
                ...
                if (leftHeader) {
                    leftHeader.css(
                        ...
                        {visibility: 'hidden'} :
                        {
                            ...
                            left: Math.min(Math.max(0, tableLeft - scrollLeft), tableRight - scrollLeft) + offsetLeft
                        });
                }
